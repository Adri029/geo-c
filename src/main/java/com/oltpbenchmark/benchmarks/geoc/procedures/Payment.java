...
import com.oltpbenchmark.benchmarks.geoc.pojo.Customer;
import com.oltpbenchmark.benchmarks.geoc.pojo.District;
import com.oltpbenchmark.benchmarks.geoc.pojo.Warehouse;
...
public class Payment extends GeoCProcedure {

    private static final Logger LOG = LoggerFactory.getLogger(Payment.class);

    public SQLStmt payUpdateWhseSQL = new SQLStmt(
            "UPDATE " + GeoCConstants.TABLENAME_WAREHOUSE +
            "   SET W_YTD = W_YTD + ? " +
            " WHERE W_ID = ? ");
    ...
    public SQLStmt payUpdateCustBalCdataSQL = new SQLStmt(
            "UPDATE " + GeoCConstants.TABLENAME_CUSTOMER +
            "   SET C_BALANCE = ?, " +
            "       C_YTD_PAYMENT = ?, " +
            "       C_PAYMENT_CNT = ?, " +
            "       C_DATA = ? " +
    ...
    public SQLStmt payInsertHistSQL = new SQLStmt(
            "INSERT INTO " + GeoCConstants.TABLENAME_HISTORY +
    ...
    public void run(Connection conn, Random gen, int w_id, int numWarehouses, int terminalDistrictLowerID, int terminalDistrictUpperID, GeoCWorker worker) throws SQLException {

        int districtID = GeoCUtil.randomNumber(terminalDistrictLowerID, terminalDistrictUpperID, gen);

        float paymentAmount = (float) (GeoCUtil.randomNumber(100, 500000, gen) / 100.0);
        ...
        int x = GeoCUtil.randomNumber(1, 100, gen);
        ...
        if (LOG.isTraceEnabled()) {
            ...
            terminalMessage.append("\n Date: ").append(GeoCUtil.getCurrentTime());
            terminalMessage.append("\n\n Warehouse: ");
    ...
    private int getCustomerWarehouseID(Random gen, int w_id, int numWarehouses, int x) {
        ...
        if (x <= 85) {
            ...
            do {
                customerWarehouseID = GeoCUtil.randomNumber(1, numWarehouses, gen);
            }
    ...
    private int getCustomerDistrictId(Random gen, int districtID, int x) {
        if (x <= 85) {
            ...
            return GeoCUtil.randomNumber(1, GeoCConfig.configDistPerWhse, gen);
        }
    ...
    private Customer getCustomer(Connection conn, Random gen, int customerDistrictID, int customerWarehouseID, float paymentAmount) throws SQLException {
        int y = GeoCUtil.randomNumber(1, 100, gen);

        Customer c;

        if (y <= 60) {
            // 60% lookups by last name
            c = getCustomerByName(customerWarehouseID, customerDistrictID, GeoCUtil.getNonUniformRandomLastNameForRun(gen), conn);
        } else {
            // 40% lookups by customer ID
            c = getCustomerById(customerWarehouseID, customerDistrictID, GeoCUtil.getCustomerID(gen), conn);
        }
    ...
    public Customer getCustomerById(int c_w_id, int c_d_id, int c_id, Connection conn) throws SQLException {
        ...
        try (PreparedStatement payGetCust = this.getPreparedStatement(conn, payGetCustSQL)) {
            ...
            try (ResultSet rs = payGetCust.executeQuery()) {
                ...
                Customer c = GeoCUtil.newCustomerFromResults(rs);
                c.c_id = c_id;
