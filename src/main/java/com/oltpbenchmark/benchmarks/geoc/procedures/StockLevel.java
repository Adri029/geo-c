...
public class StockLevel extends GeoCProcedure {
    ...
    public SQLStmt stockGetCountStockSQL = new SQLStmt(
            ...
            " FROM " + GeoCConstants.TABLENAME_ORDERLINE + ", " + GeoCConstants.TABLENAME_STOCK +
            " WHERE OL_W_ID = ?" +
            " AND OL_D_ID = ?" +
            " AND OL_O_ID < ?" +
            " AND OL_O_ID >= ?" +
    ...
    public void run(Connection conn, Random gen, int w_id, int numWarehouses, int terminalDistrictLowerID, int terminalDistrictUpperID, GeoCWorker w) throws SQLException {
        ...
        int threshold = GeoCUtil.randomNumber(10, 20, gen);
