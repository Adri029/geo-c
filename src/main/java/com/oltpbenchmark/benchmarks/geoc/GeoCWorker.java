...
import com.oltpbenchmark.benchmarks.geoc.procedures.GeoCProcedure;
...
public class GeoCWorker extends Worker<GeoCBenchmark> {

    private static final Logger LOG = LoggerFactory.getLogger(GeoCWorker.class);
    ...
    public GeoCWorker(GeoCBenchmark benchmarkModule, int id,
    ...
    /**
     * Executes a single GeoC transaction of type transactionType.
     */
    @Override
    protected TransactionStatus executeWork(Connection conn, TransactionType nextTransaction) throws UserAbortException, SQLException {
        try {
            GeoCProcedure proc = (GeoCProcedure) this.getProcedure(nextTransaction.getProcedureClass());
