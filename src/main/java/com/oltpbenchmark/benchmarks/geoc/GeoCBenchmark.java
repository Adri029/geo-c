...
public class GeoCBenchmark extends BenchmarkModule {
    private static final Logger LOG = LoggerFactory.getLogger(GeoCBenchmark.class);

    public GeoCBenchmark(WorkloadConfiguration workConf) {
        super(workConf);
    }
    ...
    protected List<Worker<? extends BenchmarkModule>> makeWorkersImpl() {
        ...
        try {
            List<GeoCWorker> terminals = createTerminals();
    ...
    @Override
    protected Loader<GeoCBenchmark> makeLoaderImpl() {
        return new GeoCLoader(this);
    }

    protected List<GeoCWorker> createTerminals() throws SQLException {

        GeoCWorker[] terminals = new GeoCWorker[workConf.getTerminals()];
        ...
        for (int w = 0; w < numWarehouses; w++) {
            ...
            final double districtsPerTerminal = GeoCConfig.configDistPerWhse / (double) numWarehouseTerminals;
            for (int terminalId = 0; terminalId < numWarehouseTerminals; terminalId++) {
                ...
                if (terminalId + 1 == numWarehouseTerminals) {
                    upperDistrictId = GeoCConfig.configDistPerWhse;
                }
                lowerDistrictId += 1;

                GeoCWorker terminal = new GeoCWorker(this, workerId++, w_id, lowerDistrictId, upperDistrictId, numWarehouses);
