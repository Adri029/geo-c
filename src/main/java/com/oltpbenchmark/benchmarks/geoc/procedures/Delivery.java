...
public class Delivery extends GeoCProcedure {
    ...
    public SQLStmt delivDeleteNewOrderSQL = new SQLStmt(
            "DELETE FROM " + GeoCConstants.TABLENAME_NEWORDER +
            " WHERE NO_O_ID = ? " +
            "   AND NO_D_ID = ?" +
            "   AND NO_W_ID = ?");

    public SQLStmt delivGetCustIdSQL = new SQLStmt(
            "SELECT O_C_ID FROM " + GeoCConstants.TABLENAME_OPENORDER +
            " WHERE O_ID = ? " +
            "   AND O_D_ID = ? " +
            "   AND O_W_ID = ?");

    public SQLStmt delivUpdateCarrierIdSQL = new SQLStmt(
            "UPDATE " + GeoCConstants.TABLENAME_OPENORDER +
            "   SET O_CARRIER_ID = ? " +
            " WHERE O_ID = ? " +
            "   AND O_D_ID = ?" +
            "   AND O_W_ID = ?");
    ...
    public SQLStmt delivUpdateDeliveryDateSQL = new SQLStmt(
            "UPDATE " + GeoCConstants.TABLENAME_ORDERLINE +
            "   SET OL_DELIVERY_D = ? " +
            " WHERE OL_O_ID = ? " +
            "   AND OL_D_ID = ? " +
            "   AND OL_W_ID = ? ");
    ...
    public SQLStmt delivSumOrderAmountSQL = new SQLStmt(
            ...
            "  FROM " + GeoCConstants.TABLENAME_ORDERLINE +
            " WHERE OL_O_ID = ? " +
            "   AND OL_D_ID = ? " +
            "   AND OL_W_ID = ?");
    ...
    public void run(Connection conn, Random gen, int w_id, int numWarehouses, int terminalDistrictLowerID, int terminalDistrictUpperID, GeoCWorker w) throws SQLException {

        int o_carrier_id = GeoCUtil.randomNumber(1, 10, gen);
        ...
        if (LOG.isTraceEnabled()) {
            ...
            terminalMessage.append(GeoCUtil.getCurrentTime());
            terminalMessage.append("\n\n Warehouse: ");
            terminalMessage.append(w_id);
            terminalMessage.append("\n Carrier:   ");
            terminalMessage.append(o_carrier_id);
            terminalMessage.append("\n\n Delivered Orders\n");
            for (int i = 1; i <= GeoCConfig.configDistPerWhse; i++) {
                if (orderIDs[i - 1] >= 0) {
