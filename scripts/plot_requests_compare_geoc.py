...
for diff, (database, benchmark, index) in enumerate(charts):
    ...
    json_paths = glob.glob(
        os.path.join(
            ...
            database,
            benchmark,
            args.results_type if diff < 3 else f"{args.results_type}-95p5",
            "*-histograms.json",
        ),
        recursive=True,
    )
    ...
    thingy = args.results_type if diff < 3 else f"{args.results_type}-95p5"
    axs[index].set_title(f"{database} / {benchmark} / {thingy}")
