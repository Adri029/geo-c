...
import com.oltpbenchmark.benchmarks.geoc.pojo.Oorder;
...
public class OrderStatus extends GeoCProcedure {
    ...
    public SQLStmt ordStatGetNewestOrdSQL = new SQLStmt(
            ...
            "  FROM " + GeoCConstants.TABLENAME_OPENORDER +
            " WHERE O_W_ID = ? " +
            "   AND O_D_ID = ? " +
            "   AND O_C_ID = ? " +
    ...
    public void run(Connection conn, Random gen, int w_id, int numWarehouses, int terminalDistrictLowerID, int terminalDistrictUpperID, GeoCWorker w) throws SQLException {
        ...
        if (y <= 60) {
            c_by_name = true;
            c_last = GeoCUtil.getNonUniformRandomLastNameForRun(gen);
        } else {
            c_by_name = false;
            c_id = GeoCUtil.getCustomerID(gen);
        }
        ...
        if (LOG.isTraceEnabled()) {
            ...
            sb.append(" Date: ");
            sb.append(GeoCUtil.getCurrentTime());
            sb.append("\n\n Warehouse: ");
            sb.append(w_id);
            sb.append("\n District:  ");
            sb.append(d_id);
        ...
        try (PreparedStatement ordStatGetOrderLines = this.getPreparedStatement(conn, ordStatGetOrderLinesSQL)) {
            ...
            try (ResultSet rs = ordStatGetOrderLines.executeQuery()) {
                ...
                while (rs.next()) {
                    ...
                    sb.append(" - ");
                    sb.append(GeoCUtil.formattedDouble(rs.getDouble("OL_AMOUNT")));
                    sb.append(" - ");
